// =========================================================
// CAMU SERVICES — APPLICATION PRINCIPALE (CORRIGÉE)
// =========================================================
...
const App = {
  ...
  async onSectionOpened(section) {
    ...
    if (section === "administration") this.loadAdministration();
  ...
  setupAuthentication() {
    ...
    forgotBtn?.addEventListener("click", () => {
      this.showToast("La récupération du mot de passe sera ajoutée prochainement.", "info");
    });
  ...
  async loadListings() {
    ...
    try {
      ...
      if (listings.length === 0) {
        const msg = `<div class="empty-message">Aucune annonce disponible. Soyez le premier à publier !</div>`;
        if (sponsored) sponsored.innerHTML = msg;
  ...
  createListingCard(listing) {
    ...
    const description = this.escapeHTML((listing.description || "").substring(0, 100));
  ...
  setupProfile() {
    ...
    form?.addEventListener("submit", async (event) => {
      ...
      this.showToast("La modification complète du profil sera connectée à Firestore dans la prochaine étape.", "info");
    });
  },
  ...
  loadChat() {
    ...
    list.innerHTML = `<div class="empty-message">Aucune conversation pour le moment.</div>`;
  },
  ...
  loadAdministration() {
    ...
    console.log("Administration autorisée.");
  },
